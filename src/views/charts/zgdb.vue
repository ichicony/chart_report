...
<div>
    ...
    <el-form-item label="类型">
      <el-select v-model="ltype" placeholder="请选择">
        <el-option
          v-for="item in types"
          :key="item"
          :value="item">
...
export default {
  ...
  methods:{
    onSubmit(){
      ...
      this.shopNm=deptnm
      this.title=this.query.year+'年'+deptnm+'专柜VS'+this.ltype+'平均'
      ...
      let url='api/htxshz/zg_lcavg'
      if(this.ltype=='部类'){
        url='api/htxshz/zg_blavg'
      }
      this.$http.post(url,{deptid:this.query.deptid[3],year:this.query.year})
      .then(res=>{
